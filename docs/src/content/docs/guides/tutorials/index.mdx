...
### Prerequisites
We assume you have configured the CodeTransEngine and vLLM server. We will use the ```examples/quickstart.example.yaml``` configuration located at the [root folder of the repository](https://anonymizedsubmissionlink/codetransengine/blob/main/quickstart.example.yaml), please refer to the [Configuration](/CodeTransEngine/guides/configuration/) guide and set-up **Python Executor Container**.
...
The ```results``` variable now holds an instance of [BatchTranslationResponse](https://anonymizedsubmissionlink/codetransengine/blob/0f21b23d49e88d7f2b159c6bfdd210897c5ae21b/protos/protos.proto#L118) that holds the ```TranslationResponse``` of every translation request.
...
🔗 [📒 quickstart.ipynb](https://anonymizedsubmissionlink/codetransengine/blob/main/examples/quickstart.ipynb)
