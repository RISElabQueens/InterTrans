...
### Prerequisites
We assume you have configured the CodeTransEngine and an OpenAI API key. We will use the ```examples/quickstart.openai.example.yaml``` configuration located at the [root folder of the repository](https://github.com/RISElabQueens/codetransengine/blob/main/quickstart.example.yaml), please refer to the [Configuration](/CodeTransEngine/guides/configuration/) guide and set-up **Python Executor Container**.
...
```bash
    go run codetransengine.go runserver quickstart.openai.example.yaml
```
...
First we will import the protobuf objects necessary to create the request and utillity functions from the CodeTransEngine Python client. 
```python
import codetransengine.protos_pb2 as ptpb
from codetransengine.utils import submit_request
```
...
The ```results``` variable now holds an instance of [BatchTranslationResponse](https://github.com/RISElabQueens/codetransengine/blob/0f21b23d49e88d7f2b159c6bfdd210897c5ae21b/protos/protos.proto#L118) that holds the ```TranslationResponse``` of every translation request.
...
#### Conclusion
As you can see, most of the work is on creating the requests, while CodeTransEngine saves the effort of running inference, executing the code and validating it. If you are translating a dataset, you can create a reusable function that creates the batch request and submits it to the server. This way you can easily translate multiple code snippets in a single run.

You can get this code here:
  
🔗 [📒 quickstart.openapi.ipynb](https://github.com/RISElabQueens/codetransengine/blob/main/examples/quickstart.openai.ipynb)
