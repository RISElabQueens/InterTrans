...
### Prerequisites
We assume you have configured the CodeTransEngine and vLLM server. We will use the ```benchmark.example.yaml``` configuration located at the [root folder of the repository](https://anonymizedsubmissionlink/codetransengine/blob/main/quickstart.example.yaml), please refer to the [Configuration](/CodeTransEngine/guides/configuration/) guide and set-up **Python Executor Container**.

#### Dataset
[HumanEval-X](https://github.com/THUDM/CodeGeeX/blob/main/codegeex/benchmark/README.md) extends the python-only code generation evaluation dataset [HumanEval](https://github.com/openai/human-eval) with additional canonical solutions and test cases in six PLs: C++, Go, Java, JavaScript, Python, and Rust. We create a subset of 1,050 translation problems, stratified across the 30 source-target translation pairs. This subset is used in the [CodeTransEngine paper](https://icse2025.hotcrp.com/doc/icse2025-paper1926-revised_manuscript.pdf) and will be used in this tutorial. **For simplicity, we will evaluate a model on a single translation pair** (Java to Python), but if you set-up all the executor containers in your configuration, you can evaluate the model on all the translation pairs.
...
```
<Aside>You can use the OpenAI API or any other compatible API as the inference backend instead of vLLM. Please find an example in the [Quickstart with OpenAI](/CodeTransEngine/guides/tutorials/openai/) section.</Aside>
...
```bash
    go run codetransengine.go runserver examples/quickstart.example.yaml
```
...
Simular to the [Quickstart](/CodeTransEngine/guides/tutorials/) guide, we will create a batch translation request with individual requests for each Java to Python translation pair sample in the benchmark.
...
```python
from codetransengine.data import response_to_pandas
...
```python
from codetransengine.data import get_ca_metric
...
🔗 [📒 benchmarking.ipynb](https://anonymizedsubmissionlink/codetransengine/blob/main/examples/benchmarking.ipynb)
